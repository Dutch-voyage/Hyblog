...
<SvizEmbed
  ...
  visualizationId="matrix-example"
  caption="矩阵示例：选择节点可查看编译后的结构，也可以缩放形状并检查默认布局。"
/>
...
<SvizEmbed
  ...
  caption="嵌套示例：Machine 包含 Processor 与 Memory，Core 0 跨层连接到 Memory。"
/>
...
在内置编辑器中选择「Demo」后，可以上传 `compile()` 生成的 JSON，现场检查交互效果，再把 JSON 与自动生成的 MDX 一起提交为 Pull Request。
...
CLI 默认创建草稿，并在目标文件已经存在时停止；只有明确传入 `--force` 才会替换已有内容。
